...
$(function() {
  ...
  midi.enable(function(err) {
    ...
    const renderStave = ({ keys, signature }) => {
      ...
      let lineRight = new Vex.Flow.StaveConnector(
        topStaff,
        bottomStaff
      ).setType(6);
      let lineLeft = new Vex.Flow.StaveConnector(topStaff, bottomStaff).setType(
        1
      );
      ...
      let alteredNotes = Key.alteredNotes(`${signature.id} major`).map(n =>
        n.toLowerCase()
      );
